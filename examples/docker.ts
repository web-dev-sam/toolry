...
export const up = defineTool({
  ...
  run: ({ detach, build }) => `docker compose up${detach ? " -d" : ""}${build ? " --build" : ""}`,
});
...
export const deploy = defineTool({
  name: "deploy",
  description: "Deploy from a build directory",
  args: {
    buildDir: {
      type: "path",
      description: "Select build output directory",
      directory: true,
      root: process.cwd(),
      required: true,
    },
  ...
  run: ({ buildDir, configFile }) => `deploy --dir ${buildDir} --config ${configFile}`,
});
